...
export function AutomatoFinito() {
  ...
  const [modalAddTransitionIsOpen, setModalAddTransitionIsOpen] =
    useState(false);

  const handleAddTransitionModal = () => {
    setModalAddTransitionIsOpen((prev) => !prev);
  };
  const [modalRemoveTransitionIsOpen, setModalRemoveTransitionIsOpen] =
    useState(false);
  ...
  useEffect(() => {
    const handleGraphvizString = () => {
      ...
      statesArray.forEach((element, index) => {
        const nodeColor = element.start
          ? "#FFDD00"
          : colors[index % colors.length]; // Cor chamativa para o estado inicial
        string += `${element.name} [shape=${
          element.final ? "doublecircle" : "circle"
  ...
  const addNewState = () => {
    ...
    if (value !== "") {
      ...
      if (aux) {
        let statesArray = [
          ...states,
          { id: countState, name: value, start: false, final: false },
        ];
  ...
  const addNewTransition = () => {
    ...
    if (value !== "") {
      ...
      for (const element of transitionsArray) {
        aux.push({
          origin_state: element.origin_state,
          input_char: element.input_char,
          dest_state: element.dest_state,
        });
      }
      ...
      for (let i = 0; i < aux.length; i++) {
        ...
        transitionsArray.push({
          id: id,
          origin_state: aux[i].origin_state,
          input_char: aux[i].input_char,
          dest_state: aux[i].dest_state,
        });
      }
  ...
  const removeState = () => {
    ...
    for (const element of transitions) {
      if (
        element.dest_state !== stateName &&
        element.origin_state !== stateName
      ) {
        values.push(element);
      }
    }
  ...
  const removeTransition = () => {
    let transitionId = parseInt(
      document.querySelector("#remove-transition-input").value
    );
    let values = [];
    for (const element of transitions) {
  ...
  const AFtoGLUD = () => {
    ...
    for (const element of transitionsAF) {
      let rules = grammar.find(
        (row) => row.nonterminal === element.origin_state
      );
      if (!rules) {
        if (element.input_char === "λ")
          grammar.push({
            nonterminal: element.origin_state,
            terminal: [element.dest_state],
          });
        else
          grammar.push({
            nonterminal: element.origin_state,
            terminal: [element.input_char + element.dest_state],
          });
      } else if (element.input_char === "λ")
        rules.terminal.push(element.dest_state);
  ...
  const validateRule = (grammar, rule, string, currNode) => {
    ...
    if (
      proxRule === "λ" &&
      rule.slice(0, rule.length - 1) === string &&
      rule.slice(0, rule.length - 1).length === string.length
    ) {
      return true;
    }
    ...
    if (
      rule.length > 1 &&
      rule.slice(0, rule.length - 1) !== string.slice(0, rule.length - 1)
    )
      return false;
    ...
    for (let r of rules.terminal) {
      stepString.push(
        `Estado atual: ${currState.nodeName} => Caractere Atual: ${receives} => Próximo estado: ${nextNode.nodeName}`
      );
        ...
        <div className="container">
          ...
          >
            <div className="click-node-box">
              ...
              <div className="checkbox-box">
                <div className="checkbox-container">
                  <input
                    ...
                    id="start-checkbox"
                    checked={startCheck}
                    onChange={() => setStartCheck(!startCheck)}
                  />
                ...
                <div className="checkbox-container">
                  <input
                    ...
                    id="final-checkbox"
                    checked={finalCheck}
                    onChange={() => setFinalCheck(!finalCheck)}
                  />
              ...
              <div className="click-node-buttons">
                <button
                  className="click-node-button"
                  onClick={handleClickedNodeChanges}
                >
                  Confirmar
                </button>
                <button
                  className="cancel-node-button"
                  onClick={handleClickNodeModal}
                >
                  Cancelar
                </button>
              </div>
          ...
          >
            <div className="add-state-box">
              ...
              <div className="add-state-buttons">
                <button className="add-state-button" onClick={addNewState}>
                  Adicionar
                </button>
                <button
                  className="cancel-state-button"
                  onClick={handleAddStateModal}
                >
                  Cancelar
                </button>
              </div>
          ...
          >
            <div className="add-transition-box">
              ...
              <div className="add-transition-buttons">
                ...
                <button
                  className="cancel-transition-button"
                  onClick={handleAddTransitionModal}
                >
                  Cancelar
                </button>
              </div>
          ...
          >
            <div className="remove-state-box">
              ...
              <div className="remove-state-buttons">
                <button className="remove-state-button" onClick={removeState}>
                  Remover
                </button>
                <button
                  className="cancel-state-button"
                  onClick={handleRemoveStateModal}
                >
                  Cancelar
                </button>
              </div>
          ...
          >
            <div className="remove-transition-box">
              ...
              <div className="remove-transition-buttons">
                ...
                <button
                  className="cancel-transition-button"
                  onClick={handleRemoveTransitionModal}
                >
                  Cancelar
                </button>
              </div>
          ...
          <div className="page-content">
            <div className="automaton-menu">
              <button className="menu-button" onClick={handleAddStateModal}>
                Adicionar Estado
              </button>
              <button
            ...
            <div id="automaton" className="automaton-box">
              <Graphviz
                options={{ fit: true, scale: 0.6, zoom: false }}
                dot={graphString}
              />
            </div>
            ...
            <div className="select-test">
              <button
                className="single-test select is-active"
                onClick={(event) => changeTestType(event)}
              >
                Entrada única
              </button>
              <button
                className="multi-test select"
                onClick={(event) => changeTestType(event)}
              >
                Entradas múltiplas
              </button>
            ...
            <div className="test-box">
              <div className="single-box">
                <label id="automaton-label" htmlFor="automaton-input">
                  {" "}
                  TESTAR STRING
                </label>
                <input
                  type="text"
                  name="automaton-input"
                  placeholder="insira sua string de teste aqui"
                  onChange={(event) =>
                    setSingleTest({ string: event.target.value, accepted: "" })
                  }
                  className={
                    `${singleTest.accepted === true ? "accepted" : ""}` ||
                    `${singleTest.accepted === false ? "rejected" : ""}`
                  }
                />
                <div className="single-buttons">
                  <button
                    className="test-button"
                    onClick={() => validateAF(singleTest)}
                  >
                    Testar
                  </button>
                </div>
              ...
              <div className="multi-box">
                <div className="tests-container">
                    ...
                    <div className="string-container" key={test.id}>
                      ...
                      >
                        ...
                        <input
                          ...
                          name="string"
                          className={
                            `${test.accepted === true ? "accepted" : ""}` ||
                            `${test.accepted === false ? "rejected" : ""}`
                          }
                          onChange={(event) =>
                            handleChangeInput(test.id, event)
                          }
                          placeholder="insira sua string de teste aqui"
                        />
                      </div>
                  ...
                  <div className="mult-buttons">
                    <button className="test-button" onClick={handleMultTest}>
                      Testar
                    </button>
                    <button className="add-button" onClick={handleAddTests}>
                      + Teste
                    </button>
                  </div>
                </div>
