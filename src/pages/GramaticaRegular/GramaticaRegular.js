...
export function GramaticaRegular() {
    ...
    const newInputFields = rules.map((i) => {
      if (id === i.id) {
        if (typeof event.target.value === "undefined") {
          ...
          i[event.target.name] = isAlpha(event.target.value.toUpperCase())
    ...
    const newInputFields = rules.map((i) => {
      if (id === i.id) {
        if (typeof event.target.value === "undefined") {
          ...
          i[event.target.name] = isAlphaNumeric(event.target.value)
            ? event.target.value
            : "";
        }
      }
  ...
  const handleGrammar = () => {
    ...
    for (let i = 0; i < nonterminal.length; i++) {
      grammarArray.push({
        id: i + 1,
        nonterminal: nonterminal[i],
        terminal: terminalList[i],
      });
    }
  ...
  const grammarValidate = (string) => {
    if (aux) {
      ...
      if (
        validator.filter((validateField) => validateField === "D").length ===
        validator.length
      ) {
        ...
        validator.filter((validateField) => validateField === "E").length ===
        validator.length
      ) {
        for (let rule of grammar[0].terminal) {
  ...
  const validate = (grammar, rule, string, type) => {
    ...
    if (type === "D") {
      ...
      if (
        proxRule === "ε" &&
        rule.slice(1, rule.length) === string &&
        rule.slice(1, rule.length).length === string.length
      )
        return true;
      ...
      if (
        ...
        rule.slice(1, rule.length) !==
          string.slice(string.length - (rule.length - 1), string.length)
      )
        return false;
    }
      ...
      <div className="container">
        ...
        <div className="page-content">
          <div className="instructions-container">
            <p>
              Insira sua Gramática Regular (GLUD ou GLUE) nos campos abaixo. O
              símbolo inicial já está preenchido automaticamente.
            </p>
            <p>
              Preencha o lado esquerdo com o símbolo não-terminal de cada
              produção, representando as regras de transformação.
            </p>
            <p>
              Utilize "ε" (épsilon) para indicar uma produção vazia, quando
              necessário.
            </p>
          </div>
          ...
          <div className="grammar-container">
              ...
              <div className="production-container" key={rule.id}>
                <div className="production-rule">
                  <input
                    ...
                    onChange={(event) => handleChangeNtInput(rule.id, event)}
                    maxLength={1}
                  />
                  <p className="arrow">➜</p>
                  <input
                    ...
                    type="text"
                    value={rule.terminal}
                    onChange={(event) => handleChangeInput(rule.id, event)}
                    placeholder="ε"
                    maxLength={2}
                  />
                </div>
                <div
                  className={`error-sintax ${
                    rule.terminal === rule.nonterminal ? "block" : "inactive"
                  }`}
                >
                  Erro de Sintaxe
                </div>
            ...
            <button className="addButton" onClick={handleAddRules}>
              + Adicionar Regra
            </button>
          </div>
          ...
          <div className="test-container">
            ...
            <div className="table-container">
              <div className="textarea-input">
                <span>insira as strings de teste aqui, uma por linha</span>
                <textarea
                  rows="15"
                  cols="33"
                  disabled={!aux}
                  ...
                  <tbody>
                    {strings.map((string, index) => (
                      <tr
                        key={string}
                        className={
                          grammarValidate(string) ? "accepted" : "rejected"
                        }
                      >
                        <td>{index + 1}</td>
                        <td>{`"${string}"`}</td>
                        <td>{grammarValidate(string) ? "sim" : "não"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
